...
```json
{
  ...
  "last_activated": "ISO-8601-Date", // Last Activated: 上次激活时间
  "created_at": "ISO-8601-Date",     // Created At: 创建时间
  "updated_at": "ISO-8601-Date"      // Updated At: 更新时间
}
```
...
### 4. Inputs (原始输入层)
- 📁 `inputs/YYYY/MM/*.md`
  Raw text or search results from the Harvester. These files are **immutable** events.
  > 来自雷达的原始文本或搜索结果。这些文件是**不可变**的事件快照。
