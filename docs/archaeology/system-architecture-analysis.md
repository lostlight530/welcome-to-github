...
- **Omniscient AST Scanner**: Deep ingestion of internal codebase using Python's standard `ast` library to extract features without external dependencies.
...
- **Intent-Driven Harvester**: Radar system (`harvester.py`) that fetches external ecosystem data with Double-Clutch Anti-Shake (ETag + SHA-256 + Diff) to avoid redundant updates.
...
## 2. 技术栈 (Tech Stack)
- **Backend/Core Logic**: Pure Python 3.10 standard library (`urllib`, `sqlite3`, `ast`, `re`). Zero external dependencies (Phase IV Absolute Determinism).
...
- **Liquid Time-Series Graph Injection**: Utilizing `concurrent.futures` with standard `urllib` to handle high-throughput external API fetching.
- **Polyglot Lexer**: The `scholar.py` regex state machine approach for parsing ASTs of JS/TS without relying on external C-bindings.
- **AST Hot-Patching**: The `evolution.py` sandbox mechanism for allowing code to modify its own source (`ast.NodeTransformer`) securely before human review.
...
- **Cross-Repo Subconscious Links**: If the system connects to the wider GitHub firehose (or specialized APIs), the graph could infer mathematical similarities between completely disparate projects based on structural AST overlaps, rather than keyword matching.
