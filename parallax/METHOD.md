...
- 使用 ISO 周期标识, 每个周期最多一个审计文件
- 复核每日专题, 特殊专题, 研究批次, 执行窗口, 反例与状态门槛
- 审计不是研究批次, 不增加 Trial 或独立执行窗口
- 同一周期重复运行只完善原审计文件
- 动态证据维护必须记录复核日期, 快照状态与是否影响原判断
- 审计不能降低候选, 发现或失效门槛
...
- audits/README.md 保存周期审计入口
- 日记录使用 templates/daily.md
- 特殊专题使用 templates/special.md
- 周期审计使用 templates/weekly.md
...
- 每份新记录声明 Record Provenance,逻辑归属日期,实际执行日期和独立时间窗口
- 同日多个 Trial 只构成一个独立时间窗口
- 来源独立性按原始发布者和证据链判断,转载不构成新来源
- 无法取得原始来源或支持命题的权威页面时,结论保持拒绝或无结论
- 周期审计只复核既有记录,不增加实验数量或长期结论数量
- 新证据不能伪装成原始观测. 已确认的正文错误允许原位微调,记录真实修正时间、原始版本、前后差异与依据,保留执行事实
